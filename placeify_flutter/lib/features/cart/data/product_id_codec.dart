...
abstract final class ProductIdCodec {
  static int? toDatabaseId(String productId) {
    ...
    if (raw.startsWith('shop-')) {
      final withoutPrefix = raw.substring('shop-'.length);
      final dash = withoutPrefix.lastIndexOf('-');
      if (dash > 0 && dash < withoutPrefix.length - 1) {
        return toDatabaseId(withoutPrefix.substring(dash + 1));
      }
    }
  ...
  static String normalizeUiProductId(String productId) {
    ...
    if (raw.startsWith('shop-')) {
      ...
      final dash = withoutPrefix.lastIndexOf('-');
      if (dash > 0 && dash < withoutPrefix.length - 1) {
        return normalizeUiProductId(withoutPrefix.substring(dash + 1));
      }
    }
