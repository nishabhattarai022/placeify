...
abstract final class CatalogSeed {
  ...
  /// Pre–p18 catalog rows superseded by [demoProductNames]; hidden from browse.
  static const legacyOrphanProductNames = [
    'Harmony',
    'Luna Sofa',
    'Floor Lamp Oden',
    'Nordic Side Table',
  ...
  static Future<void> ensureDemoCatalog(Session session) async {
    ...
    if (count > 0) {
      ...
      await reconcileLegacyProducts(session);
  ...
  static Future<void> reconcileLegacyProducts(Session session) async {
    for (final name in legacyOrphanProductNames) {
      final product = await Product.db.findFirstRow(
        session,
        where: (row) => row.name.equals(name),
      );
      if (product == null) continue;
      if (product.status == ProductStatus.removed) continue;
  ...
  static Future<void> ensureSeedProductImages(Session session) async {
    for (final seed in _demoProducts) {
      ...
      final staleThumbnail = !_usesCanonicalSeedFile(
  ...
  static bool _usesCanonicalSeedFile(String? url, String fileName) {
    if (url == null || url.trim().isEmpty) return false;
    return url.trim().endsWith('/$fileName');
  }
