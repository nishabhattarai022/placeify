...
class EmailVerificationService {
  ...
  Future<void> issueMagicLink(
    ...
    // #region agent log
    try {
      final verifyUri = Uri.tryParse(verifyUrl);
      File(
        ...
        '${jsonEncode({
          ...
          'runId': 'verify-e2e',
          'hypothesisId': 'B',
          'location': 'email_verification_service.dart:issueMagicLink',
          'message': 'magic link issued',
          'data': {
            'emailDomain': normalizedEmail.contains('@')
                ? normalizedEmail.split('@').last
                : 'unknown',
            'hasAccountRequestId': true,
            'tokenLen': rawToken.length,
            'expiresInHours': tokenExpiry.inHours,
            'linkHost': verifyUri?.host,
            'linkPort': verifyUri?.hasPort == true ? verifyUri!.port : null,
            'linkPath': verifyUri?.path,
            'hasTokenQuery': verifyUri?.queryParameters.containsKey('token'),
            'isLocalhostLink': verifyUri?.host == 'localhost' ||
                verifyUri?.host == '127.0.0.1',
            'isLanLink': verifyUri?.host.startsWith('192.168.') == true ||
                verifyUri?.host.startsWith('10.') == true,
            'isHttpsPublic': verifyUri?.scheme == 'https' &&
                verifyUri?.host.contains('trycloudflare.com') == true,
            'runIdTag': 'cloudflare-tunnel',
          },
      ...
      try {
        File(
          ...
          '${jsonEncode({
            ...
            'runId': 'verify-e2e',
            'hypothesisId': 'D',
            'location': 'email_verification_service.dart:verifyEmail',
            'message': 'pending row not found for token hash',
            'data': {'tokenLen': normalizedToken.length},
            'timestamp': DateTime.now().millisecondsSinceEpoch,
          })}\n',
          mode: FileMode.append,
          flush: true,
    ...
    try {
      File(
        ...
        '${jsonEncode({
          ...
          'hypothesisId': 'D',
          'location': 'email_verification_service.dart:verifyEmail',
          'message': 'registration finished via magic link',
          'data': {
            'tokensRevoked': true,
            'nameUpdated': trimmedName != null && trimmedName.isNotEmpty,
          },
          'timestamp': DateTime.now().millisecondsSinceEpoch,
        })}\n',
        mode: FileMode.append,
