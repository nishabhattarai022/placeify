...
abstract class Cart implements _i1.TableRow<int?>, _i1.ProtocolSerialization {
  ...
  static CartInclude include({_i2.UserInclude? user}) {
    return CartInclude._(user: user);
  }
...
class CartInclude extends _i1.IncludeObject {
  CartInclude._({_i2.UserInclude? user}) {
    _user = user;
  }
...
class CartRepository {
  ...
  /// Acquires row-level locks on [Cart] rows matching the [where] expression.
  Future<void> lockRows(
    ...
    return session.db.lockRows<Cart>(
      where: where(Cart.t),
      lockMode: lockMode,
      lockBehavior: lockBehavior,
      transaction: transaction,
...
class CartAttachRowRepository {
  ...
  /// by setting the [Cart]'s foreign key `userId` to refer to the [User].
  Future<void> user(
    _i1.DatabaseSession session,
    Cart cart,
    _i2.User user, {
    ...
    var $cart = cart.copyWith(userId: user.id);
    await session.db.updateRow<Cart>(
      $cart,
      columns: [Cart.t.userId],
      transaction: transaction,
    );
  }
}
